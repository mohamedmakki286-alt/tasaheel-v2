...
export default function LoginPage() {
  ...
  const reset = async () => {
    if (!token.trim() || newPassword.length < 6) return toast.error('تحقق من الرمز وكلمة المرور');
    setLoading(true);
    try {
    ...
    <div className="flex min-h-screen bg-gray-50 dark:bg-gray-950" dir="rtl">
      ...
      <main className="relative flex flex-1 items-center justify-center px-4 py-8 sm:px-8">
        ...
        <div className="w-full max-w-md">
          ...
          {mode === 'reset' && <><div className="mb-7"><h1 className="text-3xl font-black text-gray-950 dark:text-white">تعيين كلمة مرور جديدة</h1><p className="mt-2 text-gray-500">أدخل الرمز المرسل ثم كلمة المرور الجديدة.</p></div><div className="space-y-5"><input value={token} onChange={e=>setToken(e.target.value)} placeholder="رمز التحقق" className="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-center outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white"/><input type="password" value={newPassword} onChange={e=>setNewPassword(e.target.value)} placeholder="كلمة المرور الجديدة" className="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white"/><button onClick={reset} disabled={loading} className="w-full rounded-xl bg-red-600 px-5 py-3 font-bold text-white">{loading?'جاري الحفظ…':'حفظ كلمة المرور'}</button><button onClick={()=>setMode('login')} className="w-full text-sm font-bold text-gray-500">العودة للدخول</button></div></>}
        </div>
      </main>
    </div>
  );
