...
import { getInvoiceByRequest } from '../api/invoice.api';
import { getChatRoom, getChatMessages, sendChatMessage, markChatAsRead } from '../api/chat.api';
...
const QUOTE_STATUS_MAP: Record<string, { label: string; variant: string }> = {
  pending: { label: 'قيد المراجعة', variant: 'warning' },
  accepted: { label: 'مقبول', variant: 'success' },
  rejected: { label: 'مرفوض', variant: 'gray' },
};
...
export default function RequestDetailPage() {
  ...
  const { data: quotes = [], isLoading: quotesLoading } = useQuery({
    ...
    queryFn: () => getRequestQuotes(Number(id)),
  ...
  const { data: invoice, isLoading: invoiceLoading } = useQuery({
    queryKey: ['invoice', id],
    queryFn: () => getInvoiceByRequest(Number(id)),
    enabled: !!id && activeTab === 'invoice',
  });

  const { data: chatRoom } = useQuery({
    queryKey: ['chat-room', id],
    queryFn: () => getChatRoom(Number(id)),
  ...
  const { data: chatMessages = [], refetch: refetchMessages } = useQuery({
    ...
    queryFn: async () => {
      ...
      const msgs = await getChatMessages(String(chatRoom.id));
      await markChatAsRead(String(chatRoom.id));
      return msgs;
    },
    enabled: !!chatRoom?.id && activeTab === 'chat',
  ...
  const sendMessageMutation = useMutation({
    mutationFn: (content: string) => {
      if (!chatRoom?.id) throw new Error('No chat room');
      return sendChatMessage(String(chatRoom.id), content, 'text');
    },
    onSuccess: () => {
      setChatMessage('');
      refetchMessages();
            ...
            <div className="space-y-4">
              {quotesLoading ? (
                ...
                quotes.map((quote: any) => {
                  const statusInfo = QUOTE_STATUS_MAP[quote.status] || QUOTE_STATUS_MAP.pending;
                  return (
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full">
                          ...
                          <tbody className="divide-y divide-gray-50">
                            {inspectionReport.parts.map((part: any, idx: number) => (
                    ...
                    <div>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full">
                          ...
                          <tbody className="divide-y divide-gray-50">
                            {inspectionReport.labor.map((item: any, idx: number) => (
                              <tr key={idx} className="hover:bg-gray-50/50">
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-gray-900">{formatDate(invoice.createdAt || request.createdAt)}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-gray-900">{invoice.workshopName || request.workshopName}</p>
                    </div>
                  </div>
                        ...
                        <tbody className="divide-y divide-gray-50">
                          {invoice.items.map((item: any, idx: number) => (
                            <tr key={idx}>
                              ...
                              <td className="px-4 py-3 text-sm text-gray-700">{formatCurrency(item.unitPrice || item.price)}</td>
                              <td className="px-4 py-3 text-sm font-semibold text-gray-900">{formatCurrency(item.total || (item.quantity * (item.unitPrice || item.price)))}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <div className="space-y-2 pt-4 border-t border-gray-100">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold text-gray-900">{formatCurrency(invoice.totalAmount || invoice.total || 0)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500">{t('pages.requests.detail.invoice.tax')}</span>
                      <span className="font-semibold text-gray-900">{formatCurrency(invoice.taxAmount || invoice.tax || 0)}</span>
                    </div>
                    <div className="flex justify-between text-lg pt-2 border-t">
                      <span className="font-bold text-gray-900">{t('pages.requests.detail.invoice.total')}</span>
                      <span className="font-bold text-amber-600">{formatCurrency(invoice.grandTotal || invoice.total || 0)}</span>
                    </div>
                  </div>
                ...
                <>
                  <div className="space-y-4 max-h-[400px] overflow-y-auto mb-4">
                    {chatMessages.map((msg: any, idx: number) => {
                      ...
                      return (
                        <div key={msg.id || idx} className={clsx('flex', isAdmin ? 'justify-start' : 'justify-end')}>
                          <div className={clsx(
                            ...
                            {!isAdmin && (
                              <p className="text-xs font-semibold text-amber-600 mb-1">{msg.senderName}</p>
                            )}
                            <p className="text-sm leading-relaxed">{msg.content}</p>
                            <p className={clsx('text-[10px] mt-1', isAdmin ? 'text-white/60' : 'text-gray-400')}>
