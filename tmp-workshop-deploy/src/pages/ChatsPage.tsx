...
import { getRoom, markAsRead } from '../api/chat.api';
...
export default function ChatsPage() {
  ...
  return <div className="mx-auto w-full max-w-3xl space-y-5" dir="rtl">
    ...
    <div className="overflow-hidden rounded-3xl border border-surface-200 bg-white dark:border-surface-700 dark:bg-surface-900">
      {isLoading ? <div className="p-10 text-center text-surface-500">جارٍ تحميل المحادثات…</div> : conversations.length ? conversations.map((request) => <ChatRow key={request.id} request={request} />) : <div className="flex min-h-64 flex-col items-center justify-center p-8 text-center"><MessageCircle size={40} className="mb-3 text-surface-300"/><h2 className="font-black">لا توجد محادثات حالياً</h2><p className="mt-1 text-sm text-surface-500">تظهر المحادثات بعد قبول العميل للعرض.</p></div>}
    </div>
  </div>;
}

function ChatRow({ request }: { request: any }) {
  const { data: room } = useQuery({
    queryKey: ['chat-room', request.id],
    queryFn: () => getRoom(String(request.id)),
    enabled: !!request.id,
  });
  const unread = room?.unreadCount || 0;
  return (
    <Link to={`/requests/${request.id}/chat?customerName=${encodeURIComponent(request.customer.name)}`} className="flex items-center gap-3 border-b border-surface-100 p-4 transition hover:bg-surface-50 last:border-0 dark:border-surface-800 dark:hover:bg-surface-800/60">
