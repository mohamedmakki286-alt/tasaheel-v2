...
public class WorkshopService {
    ...
    public List<MaintenanceRequestDTO> getPendingRequests(Long workshopId, String city) {
        List<String> openStatuses = List.of("pending", "quoted");
        List<MaintenanceRequest> cityRequests = requestRepository.findByCityAndStatusIn(city, openStatuses);
        List<MaintenanceRequest> assignedRequests = requestRepository.findByPreferredWorkshopIdAndStatusIn(workshopId, openStatuses);
        List<MaintenanceRequest> merged = Stream.concat(cityRequests.stream(), assignedRequests.stream())
                .distinct()
                .filter(request -> quoteRepository.findByRequestIdAndWorkshopId(request.getId(), workshopId).isEmpty())
                .collect(Collectors.toList());
        return merged.stream().map(this::toRequestDTO).collect(Collectors.toList());
    }
