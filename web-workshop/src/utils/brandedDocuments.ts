...
export async function exportInvoiceDocument(invoice: any, context: PartyContext) {
  ...
  const meta = `<div><small>رقم الفاتورة</small><strong>${escapeHtml(invoice.invoiceNumber || invoice.id)}</strong></div><div><small>رقم الطلب</small><strong>${escapeHtml(context.requestNumber || context.requestId)}</strong></div><div><small>تاريخ الإصدار</small><strong>${date(invoice.createdAt)}</strong></div><div><small>الحالة</small><strong>${escapeHtml(invoice.status || 'صادرة')}</strong></div>`;
...
export async function exportInspectionDocument(report: any, context: PartyContext) {
  ...
  const meta = `<div><small>رقم التقرير</small><strong>${escapeHtml(report.reportNumber || report.id)}</strong></div><div><small>رقم الطلب</small><strong>${escapeHtml(context.requestNumber || context.requestId)}</strong></div><div><small>تاريخ الفحص</small><strong>${date(report.createdAt)}</strong></div><div><small>الأولوية</small><strong>${escapeHtml(report.priority || 'عادية')}</strong></div>`;
