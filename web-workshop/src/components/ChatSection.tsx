  ...
  return (
    <div>
      ...
      <form onSubmit={handleSend} className="relative flex items-center gap-2 rounded-[28px] bg-white p-1.5 shadow-[0_5px_20px_rgba(15,23,42,0.10)] ring-1 ring-surface-100 dark:bg-surface-800 dark:ring-surface-700" dir="ltr">
        ...
        {content.trim() || previewFile ? <button type="submit" disabled={mutation.isPending} className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-primary-500 text-white transition hover:bg-primary-600 disabled:opacity-50"><Send size={20} /></button> : <button type="button" onClick={() => toast('التسجيل الصوتي سيُفعّل قريباً')} className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full text-surface-900 transition hover:bg-surface-100 dark:text-white dark:hover:bg-surface-700" aria-label="رسالة صوتية"><Mic size={27} strokeWidth={2.3} /></button>}
      </form>
    </div>
  );
}
